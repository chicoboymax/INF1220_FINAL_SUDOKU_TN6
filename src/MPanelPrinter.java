...
public class MPanelPrinter implements Printable {
	...
	public void setLRMargins(int margin) {
		...
		paper.setImageableArea(paper.getImageableX() - (double) (margin / 2),
				paper.getImageableY(), paper.getImageableWidth()
						+ (double) (margin / 2), paper.getImageableHeight());
	...
	public void setTBMargins(int margin) {
		...
		paper.setImageableArea(paper.getImageableX(), paper.getImageableY()
				- (double) (margin / 2), paper.getImageableWidth(),
		...
		for (int i = 0; i < panel.getComponentCount(); i++) {
			...
			if (panel.getLayout() instanceof GridBagLayout)
				gridBagInsets = ((GridBagLayout) panel.getLayout())
						.getConstraints(panel.getComponent(i)).insets.bottom
						+ ((GridBagLayout) panel.getLayout())
								.getConstraints(panel.getComponent(i)).insets.top;
			double hauteurComponent = panel.getComponent(i).getSize()
					.getHeight()
					+ (double) gridBagInsets;
